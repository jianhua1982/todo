/**
 * Created by cup on 7/27/16.
 */

/*
 {"appId":"mchntDemo","nonceStr":"EQ1sfje7jb1k0yZK","timestamp":1469614410,"url":"http://172.17.250.156/web/public/mchnt/h5/html/sdkDemo.html","signature":"cee408c55dbc30a2d02001634395204ad54cc909"}
*/
function alert(msg) {
    console.log(msg);
    //window.alert(msg);
...
function checkSig(params) {
    if(!$.isPlainObject(params)) {
        alert('params必须是object');
        return;
    }

    if(!(params["appId"] && params["nonceStr"] && params["timestamp"] && params["signature"])) {
        alert('以上参数必填');
        return;
    }

    if(params["frontToken"]) {
        // verify sig.
        ...
        data["frontToken"] = params["frontToken"];
...
(function (global)
{
    ...
    {
        let bin = [], i, existingByteLen, intOffset, byteOffset;
    ...
    {
        let length = binarray.length * 4, i, retVal = new ArrayBuffer(length);
    ...
    {
        let i, appendedMessageLength, offset, totalLen;
     ...
     */
    let jsSHA = function(variant, inputFormat, options)
    {
        let processedLen = 0, remainder = [], remainderLen = 0, utfType,
        ...
        {
            let formatFunc, i, outputOptions, finalizedH;
        ...
        {
            let formatFunc,	firstHash, outputOptions, finalizedH;
