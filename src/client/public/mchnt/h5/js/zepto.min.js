...
;(function(undefined){
  ...
  if (Array.prototype.reduce === undefined)
    Array.prototype.reduce = function(fun){
      ...
      let t = Object(this), len = t.length >>> 0, k = 0, accumulator
...
let Zepto = (function() {
  let undefined, key, $, classList, emptyArray = [], slice = emptyArray.slice, filter = emptyArray.filter,
  ...
  zepto.fragment = function(html, name, properties) {
    ...
    let nodes, dom, container = containers[name]
  ...
  $.fn = {
    ...
    closest: function(selector, context){
      let node = this[0], collection = false
    ...
    data: function(name, value){
      let data = this.attr('data-' + dasherize(name), value)
  ...
  function traverseNode(node, fun) {
    fun(node)
    for (let key in node.childNodes) traverseNode(node.childNodes[key], fun)
  }
...
;(function($){
  let $$ = $.zepto.qsa, handlers = {}, _zid = 1, specialEvents={},
  ...
  function findHandlers(element, event, fn, selector) {
    event = parse(event)
    if (event.ns) let matcher = matcherFor(event.ns)
  ...
  function add(element, events, fn, selector, getDelegate, capture){
    let id = zid(element), set = (handlers[id] || (handlers[id] = []))
  ...
  $.proxy = function(fn, context) {
    if ($.isFunction(fn)) {
      let proxyFn = function(){ return fn.apply(context, arguments) }
      proxyFn._zid = zid(fn)
      return proxyFn
  ...
  function createProxy(event) {
    let key, proxy = { originalEvent: event }
    for (key in event)
  ...
  $.fn.delegate = function(selector, event, callback){
    return this.each(function(i, element){
      add(element, event, callback, selector, function(fn){
        return function(e){
          let evt, match = $(e.target).closest(selector, element).get(0)
          if (match) {
  ...
  $.Event = function(type, props) {
    ...
    let event = document.createEvent(specialEvents[type] || 'Events'), bubbles = true
...
;(function($, undefined){
  let prefix = '', eventPrefix, endEventName, endAnimationName,
  ...
  $.fn.anim = function(properties, duration, ease, callback){
    let key, cssValues = {}, cssProperties, transforms = '',
...
//    let xDelta = Math.abs(x1 - x2), yDelta = Math.abs(y1 - y2)
...
//})(Zepto)
;(function($){
  let zepto = $.zepto, oldQsa = zepto.qsa, oldMatches = zepto.matches
