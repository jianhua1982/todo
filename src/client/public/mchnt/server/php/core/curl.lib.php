...
class Curl {
    ...
    /*
     * $isSuccess [write], 后台是否返回成功.
     * 成功返回params 里面的内容, 失败返回errcode 和 errmsg
     */
    public static function callWebServerWithCmd($cmd, $param=array(), &$retData) {
        if(MOCK_SERVER) {
//            $queryUrl = Util::curPageURL('/web/public/mchnt/server/mockServer' . '/1.0/' . $cmd);
            $queryUrl = UPWALLET_URL . '/s/mock/1.0/' . $cmd;
        }
        else {
            $queryUrl = UPWALLET_URL . '/open/access/1.0/'.$cmd;
        }

        Util::log('++++++  $queryUrl = ' . $queryUrl);
        Util::log($param);

        $ret = self::callWebServer($queryUrl, $param, 'post', true, false);

        Util::log($ret);

        if(isset($ret['resp']) && $ret['resp'] == '00') {
            // by succcess
            if(isset($ret['params'])) {
                $retData = $ret['params'];
            }
